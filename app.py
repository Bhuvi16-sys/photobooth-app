...
/* Target the container of the WebGL iframe to hide its height and padding */
...
vSrc = fluidVertex if theme == "dark" else cloudVertex
fSrc = fluidFragment if theme == "dark" else cloudFragment
opacity_val = "0.18" if theme == "dark" else "0.55"

components.html(f"""
<body style="margin: 0; padding: 0; overflow: hidden; background: transparent;">
<canvas id="theme-webgl-bg" style="position: absolute; top: 0; left: 0; width: 100vw; height: 100vh; pointer-events: none; opacity: {opacity_val};"></canvas>
<script>
(function() {{
    const canvas = document.getElementById("theme-webgl-bg");
    if (!canvas) return;
    
    const gl = canvas.getContext("webgl", {{ antialias: true, alpha: true }});
    if (!gl) return;

    const vertexShaderGLSL = `{vSrc}`;
    const fragmentShaderGLSL = `{fSrc}`;
...
<div style="display: grid; grid-template-columns: 1fr 1.2fr; gap: 20px; align-items: center; width: 100%;">
<!-- Circle Dial Section -->
...
<!-- Attribute Stats -->
<div style="display: flex; flex-direction: column; gap: 8px; text-align: left; padding-left: 10px;">
